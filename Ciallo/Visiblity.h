...
/*
 * Modified version by ShenCiao for his personal research project Ciallo
 * Let it be applicable to polyline geometry trait.
 * Delete the condition when query point on edge, change the function name when query point in face.
 * Observer has been deleted, manually update this object.
 */
...
namespace CGAL {
    ...
    class Triangular_expansion_visibility_2 {
        typedef typename Arrangement_2_::Geometry_traits_2::Segment_traits_2    Geometry_traits_2;
    ...
    public:
        ...
        typename VARR::Face_handle
            compute_visibility_in_bounded_face(const Point_2& q,
                VARR& out_arr)
            const {
    ...
    public:
        void init_cdt() const {
            p_cdt = std::make_shared<CDT>();
            for (Edge_const_iterator it = p_arr->edges_begin(); it != p_arr->edges_end(); ++it)
            {
